...
class CantoneseDocumentarySystemPrompt(TextPrompt):
    ...
    def get_template(self) -> str:
        return """
          ...
          2. 每个片段在长视频中的时间范围
          3. 每个片段的持续时间(以秒为单位)
          4. 每个片段的原始字幕(附带说话人(如speaker0)与BGM标识)
          5. 每个片段的画面描述(这个是从片段中截取的关键图片)
          6. 每个片段的关键要素
          7. 每个片段的内容分析
          8. 每个片段的片段总结
          9. 说话人的角色表现
          10. 每个片段的与整个视频多个主题的相关度。
          
          你还精通旁白书写，能够根据具体的片段内容、时长与主题要求，撰写出符合粤语荧幕表达习惯的解说文案。

          剪辑脚本的输出格式按照用户后续提供的要求进行。
          """
...
class CantoneseDocumentaryPrompt(TextPrompt):
    ...
    def get_template(self) -> str:
      ...
      - final: 最终创作阶段，基于时长限制和叙事逻辑进行深度重构，包括：
        * 重新调整片段时长（可能裁剪候选片段）
        * 重新编写解说文案（适应新的剪辑节奏和逻辑）
        * 重新设计片段衔接（确保故事流畅性）
        * 重新计算时间戳（保证总时长在120-180秒内）
      ...
      4. 持续时间的重新计算：
        - 确保final阶段的total_duration严格控制在目标范围内
  ...
  "alternative": {
    "items": [
      ...
      },
      {
          ...
          "picture": "一个女子在厨房里忙碌，准备美食",
          "narration": "中午时分，老板好忙啊",
          "duration": 12.3,
          "total_duration": 27.9,
          "OST": 0
      },
  ...
  "final": {
    "items": [
      ...
      },
      {
          "_id": 2,
          "timestamp": "00:00:35,000-00:00:45,800",
          "picture": "精选画面片段，强化主题表达",
          "narration": "全新创作的收尾解说，呼应开场并总结主题",
          "duration": 10.8,
          "total_duration": 27.6,
          "OST": 0
      }
...
### 技术规范
- 三阶段创作流程：必须严格遵循alternative→valid→final的深度创作流程
- 候选阶段(alternative)：广泛收集素材，从video_content中筛选6-12个潜在符合主题的片段
- 验证阶段(valid)：质量把关，从候选片段中选出最优质、最符合主题的片段ID列表
- 最终阶段(final)：深度重构创作，包含以下关键要素：
  * 解说文案重写：基于已有文案，按照前面的视频逻辑重新创作解说文案
  * 叙事逻辑重构：重新设计片段间的逻辑关系和过渡方式
  * 时间戳重新计算：基于新的时长设计重新计算和total_duration
  * 画面描述优化：重新描述画面重点，突出最关键的视觉元素
- 时长计算规范：
  * alternative阶段：基于原始内容的自然时长，可以较长，为筛选提供空间
  * final阶段：经过深度思考后的优化时长，必须严格控制总时长在120-180秒内
...
5. 最终优化：确保重构后的内容在120-180秒内达到最佳观看效果
"""